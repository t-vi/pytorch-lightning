...
def test_accelerator_choice_ddp_cpu_custom_cluster(device_count_mock, setup_distributed_mock):
    ...
    model = BoringModel()
    trainer = Trainer(
        plugins=[CustomCluster()], fast_dev_run=True, accelerator="ddp_cpu", num_processes=2, callbacks=[CB()]
    )
...
def test_custom_accelerator(device_count_mock, setup_distributed_mock):
    ...
    accelerator = Accel(training_type_plugin=TrainTypePlugin(device=torch.device("cpu")), precision_plugin=Prec())
