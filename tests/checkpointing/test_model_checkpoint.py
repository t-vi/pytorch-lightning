...
def test_model_checkpoint_every_n_val_epochs(tmpdir, every_n_val_epochs):
    ...
    # check that the correct ckpts were created
    expected = (
        [f"epoch={e}.ckpt" for e in range(epochs) if not (e + 1) % every_n_val_epochs] if every_n_val_epochs > 0 else []
    )
    assert set(os.listdir(tmpdir)) == set(expected)
...
def test_model_checkpoint_save_last_warning(
    ...
    trainer = Trainer(default_root_dir=tmpdir, callbacks=[ckpt], max_epochs=max_epochs)
...
def test_model_checkpoint_save_last_checkpoint_contents(tmpdir):
    ...
    trainer = Trainer(default_root_dir=tmpdir, callbacks=[model_checkpoint], max_epochs=num_epochs)
    trainer.fit(model)
